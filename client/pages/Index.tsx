...
interface Item { id: string; title: string; description?: string; createdAt: string; updatedAt: string }
...
export default function Index() {
  ...
  const [commitMessage, setCommitMessage] = useState("feat: add code from in-app compiler");
  const [language] = useState("javascript");
  const [code, setCode] = useState<string>("console.log('Hello from in-app compiler!')");
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, title, description }: { id: string; title?: string; description?: string }) => {
      const res = await fetch(`/api/items/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title, description }) });
  ...
  const brandGradient = useMemo(() => "bg-gradient-to-br from-primary/20 via-accent/20 to-transparent", []);
      ...
      <section className="container mx-auto px-4 py-10 grid grid-cols-1 xl:grid-cols-2 gap-8">
        <Card className="border-primary/20 shadow-[0_10px_40px_-15px_rgba(99,102,241,0.4)]">
          <CardHeader>
            ...
            <CardDescription>Write JavaScript, run it safely, then commit to your GitHub repository.</CardDescription>
          </CardHeader>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div>
                  ...
                  <Input value={filePath} onChange={(e) => setFilePath(e.target.value)} placeholder="src/index.js" />
                </div>
              </div>
            </div>
        ...
        <Card className="border-accent/20">
          <CardHeader>
            ...
            <CardDescription>Create, edit, and delete items stored on the server.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              {itemsQuery.data?.items?.length ? (
                itemsQuery.data.items.map((it) => (
                  <ItemRow key={it.id} item={it} onSave={(t, d) => updateMutation.mutate({ id: it.id, title: t, description: d })} onDelete={() => deleteMutation.mutate(it.id)} />
                ))
              ) : (
                <p className="text-sm text-muted-foreground">No items yet. Add your first above.</p>
              )}
            </div>
          </CardContent>
        </Card>
...
function ItemRow({ item, onSave, onDelete }: { item: Item; onSave: (title?: string, description?: string) => void; onDelete: () => void }) {
  ...
  return (
    <div className="grid md:grid-cols-5 gap-2 items-center rounded-md border p-2">
      <Input className="md:col-span-2" value={t} onChange={(e) => setT(e.target.value)} />
      <Input className="md:col-span-2" value={d} onChange={(e) => setD(e.target.value)} />
      <div className="flex gap-2 justify-end">
        <Button size="sm" variant={changed ? "default" : "secondary"} onClick={() => onSave(t, d)} disabled={!changed}>Save</Button>
