...
export const pushFile: RequestHandler = async (req, res) => {
  try {
    ...
    if (!owner || !repo || !path || !message || typeof content !== "string") {
      return res
        .status(400)
        .json({ error: "owner, repo, path, message, content are required" });
    }
    ...
    try {
      const fileData = await githubRequest<any>(
        token,
        `${base}/contents/${encodeURIComponent(path)}?ref=${encodeURIComponent(branch)}`,
      );
      sha = fileData.sha;
    } catch {
    ...
    res.json({
      ok: true,
      content: { path: response.content?.path, sha: response.content?.sha },
